...
namespace InfoPanel.SteamAPI.Services
{
    ...
    public class LibraryDataService
    {
        ...
        public LibraryDataService(
            ...
            SessionTrackingService? sessionTrackingService = null,
        ...
        public async Task<LibraryData> CollectLibraryDataAsync()
        {
            try
            {
                ...
                // 3. Collect recent session statistics (from local history)
                if (_sessionTrackingService != null)
                {
                    var sessionStats = _sessionTrackingService.GetRecentSessionStats(14); // Last 14 days (2 weeks)
                    libraryData.RecentSessionsCount = sessionStats.sessionCount;

                    _enhancedLogger?.LogDebug("LibraryDataService.CollectLibraryDataAsync", "Collected recent session stats", new
                    {
                        SessionsLast2Weeks = libraryData.RecentSessionsCount
                    });
                }
    ...
    public class LibraryData
    {
        ...
        /// <summary>
        /// Number of gaming sessions in the last 2 weeks
        /// </summary>
